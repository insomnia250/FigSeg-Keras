...
class trainAug(object):
    def __init__(self, size=(448,448)):
        self.augment = Compose([
            ...
            ExpandBorder(mode='constant', value=255, size=size, resize=True),
...
def get_train_val(img_root, test_size=0.1, random_state=42):
    ...
    mask_paths = ['/'.join(p.split('/')[:-2]) + '/seg_mask/' + p.split('/')[-1].replace('.jpg', '.png') for p in
                  img_paths]
    anno = pd.DataFrame({'img_paths':img_paths,
...
def gen_dataloader(img_root,size=(224,224), validation_split=0.1,train_bs =8,val_bs=4):
    ...
    train_pd, val_pd = get_train_val(img_root, test_size=validation_split, random_state=42)
