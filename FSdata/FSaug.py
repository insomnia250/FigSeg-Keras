...
__all__ = ['Compose','RandomHflip', 'RandomUpperCrop', 'Resize', 'UpperCrop', 'RandomBottomCrop',"RandomErasing",
           'BottomCrop', 'Normalize', 'RandomSwapChannels', 'RandomRotate', 'RandomHShift',
           'ExpandBorder', 'RandomResizedCrop','RandomDownCrop', 'DownCrop', 'ResizedCrop', 'Hflip']
...
class RandomRotate(object):
    def __init__(self, angles, bound, select):
        self.angles = angles
        self.bound = bound
        self.select = select
...
class RandomHflip(object):
    def __call__(self, image, attr_idx):
        if random.randint(2):
            return cv2.flip(image, 1), attr_idx
        else:
            return image, attr_idx
...
class CenterCrop(object):
    ...
    def __call__(self, image, attr_idx):
        return center_crop(image, self.size), attr_idx

class UpperCrop():
    def __init__(self, size,  select,ratio=1,crop=2,nochange=0, scale=(0.09, 0.81)):
        self.size = size
        self.scale = scale
        self.ratio = ratio
        self.select = select
    ...
    def __call__(self,img, attr_idx):
        if (attr_idx not in self.select) or self.nochange:
            return img, attr_idx
...
class RandomUpperCrop(object):
    def __init__(self, size, select, scale=(0.09, 0.81), ratio=(3. / 4., 4. / 3.)):
        self.size = size
        self.scale = scale
        self.ratio = ratio
        self.select = select
...
class Resize(object):
    def __init__(self, size, select, inter=cv2.INTER_LINEAR):
        self.size = size
        self.inter = inter
        self.select = select

    def __call__(self, image, attr_idx):
        if attr_idx in self.select:
            return cv2.resize(image, (self.size[0], self.size[0]), interpolation=self.inter), attr_idx
        return image, attr_idx

class ExpandBorder(object):
    def __init__(self, select, mode='constant', value=255, size=(336,336), resize=False):
        self.select = select
        self.mode = mode
        self.value = value
        self.resize = resize
...
class Normalize(object):
    ...
    def __call__(self, image, attr_idx):
        ...
        :return:
        '''
        return (image.transpose((2, 0, 1)) / 255. - self.mean) / self.std, attr_idx

class RandomErasing(object):
    def __init__(self, select,EPSILON=0.5,sl=0.02, sh=0.09, r1=0.3, e_num=[1,10], mean=[0.485, 0.456, 0.406]):
    ...
    def __call__(self, img,attr_idx):
        ...
        if random.uniform(0, 1) > self.EPSILON:
            return img,attr_idx
        re_num=int(round(random.uniform(self.e_num[0],self.e_num[1])))
        count_e=0
        for attempt in range(100):
            area = img.shape[1] * img.shape[2]

            target_area = random.uniform(self.sl/re_num, self.sh/re_num) * area
